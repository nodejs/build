...
server {
    ...
    # We gzip the responses that NGINX does for the following mime-types. We don't use gzip_static as otherwise it would
    # request a pre-gzip version of each file on the file system, which would hammer the file system with requqests.
    gzip on;
    gzip_disable "MSIE [1-6]\.";
    gzip_types text/plain text/css application/javascript text/xml application/xml application/xml+rss image/svg+xml;

    # These directives are used to prevent our server to continuously attempt to open the same requested files on the file_system
    # W use this so that file_system accesses get cached during this period of time.
    # For specific Location directives, such as /dist we also cache the 404's and any other Server Error
    ...
    # We set a default cache of 1 hour and a max age of 4 hours
    add_header Cache-Control "public, max-age=3600, s-maxage=14400";

    error_page 404 /404.html;

    # For the unencrypted Node.js server, if a request is not done within the /dist folder
    # We basically rewrite/redirect the request to the https:// schema/protocol
    location ~ ^/(?!(dist/|dist$|\.json$)) {
        rewrite ^ https://$host$uri permanent;
    }

    # This should handle any other possible request done to the unencrypted server
    # Even tho as seen on the previous Location block for ~ we redirect all non /dist requests to https://
    location / {
    ...
    # This Location directive redirects to the list of all Node.js binaries available
    # of all existing Node.js versions
    # We use `open_file_cache_errors` that allows the NGINX file cache to also cache that
    # certain files gave 404's, so that it doesn't keep retrying to open this file that doesn't exist
    # This policy will still follow the expiry time of the file_cache. It is used mostly to avoid our file system
    # to be hammered by concurrent requests and mitigate situations where the fs stops fulfiling requests
    ...
    location = /404.html {
        # We disable caching of 404 pages as we want to avoid CloudFlare accidentally thinking that a 404 is permanent
        # many cases 404 pages happen due to the server reaching maximum concurrent file system open() requests
        # disabling cache allows CloudFlare to re-evaluate the same $uri once our server recovers and then properly cache the page
...
server {
    ...
    # This Location directive redirects to the overall available Downloads on this Server
    # We use `open_file_cache_errors` that allows the NGINX file cache to also cache that
    # certain files gave 404's, so that it doesn't keep retrying to open this file that doesn't exist
    # This policy will still follow the expiry time of the file_cache. It is used mostly to avoid our file system
    # to be hammered by concurrent requests and mitigate situations where the fs stops fulfiling requests
    ...
    # This Location directive is used for handling all 404's requests coming to this Server
    # by default if the request doesn't come from a /two-letter-code/ localized page (extracted from the @english_fallback Location)
    # it will then fallback to $lang as being "en" as defined numerous lines above
    location @localized_404 {
        ...
        # if there is no 404 page for that Localized page we will fallback to the English Website 404
        # as last resort we fallback to NGINX's default 404 on worst scenario. Notice that this result as a [crit] and should never happen
    ...
    # it does requests here from its different colos every 60s. We decided to use a direct empty response here
    # to avoid having the file system being requested for every time CloudFlare does the health_check
    # We also disabled access logs as we honestly don't need these access_logs
